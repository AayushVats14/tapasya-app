...
import { 
  ArrowLeft, Search, Plus, Shield, Users, 
  ArrowRight, Zap, Target, X, FolderHeart 
} from "lucide-react";
...
export default function CommunityPage() {
  ...
  const handleCreateSquad = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const randomJoinCode = Math.random().toString(36).substring(2, 8).toUpperCase();
      ...
      <div className="w-full max-w-5xl relative z-10 flex flex-col gap-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-2">
          <div>
            ...
            <p className="text-zinc-500 mt-2 text-sm">Join a community of like-minded aspirants and lock in together.</p>
          </div>
          ...
          <div className="flex flex-col divide-y divide-white/5 max-h-[600px] overflow-y-auto custom-scrollbar">
            {loading ? (
              <div className="py-20 text-center flex flex-col items-center gap-3">
                ...
                <p className="text-zinc-500 text-xs font-mono tracking-widest uppercase">Scanning for Squads...</p>
              </div>
              ...
              filteredSquads.map((squad) => (
                ...
                >
                  <div className="w-full md:w-3/12 flex items-center gap-3 mb-3 md:mb-0 pr-4">
                    ...
                    <span className="text-zinc-100 font-medium text-sm truncate">{squad.name || "Unnamed Squad"}</span>
                  </div>
                  ...
                  <div className="w-full md:w-4/12 mb-4 md:mb-0 pr-6">
                    <p className="text-zinc-400 text-sm font-light line-clamp-2 leading-relaxed">
                      {squad.description || "No specific objective set. Just pure focus."}
                    </p>
                  </div>

                  <div className="w-full md:w-2/12 flex items-center gap-3 mb-3 md:mb-0">
                    <div className="flex -space-x-2">
                      {[...Array(Math.min(3, squad.member_count))].map((_, i) => (
                        <div 
                          key={i} 
                  ...
                  <div className="w-full md:w-2/12 flex items-center gap-2 mb-4 md:mb-0">
                    ...
                    <span className="text-zinc-300 text-sm truncate">{squad.host_name}</span>
                  </div>
          ...
          <div className="w-full max-w-md bg-[#0c0d12]/95 border border-white/10 rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleCreateSquad} className="p-6 space-y-4">
              ...
              <div className="space-y-2">
                <label className="text-xs text-zinc-400 font-medium ml-1">Squad Name</label>
                <input 
                  type="text"
                  required
                  value={newSquadName}
              ...
              <div className="space-y-2">
                <label className="text-xs text-zinc-400 font-medium ml-1">Objective / Description (Optional)</label>
                <textarea 
                  rows={3}
                  value={newSquadDesc}
